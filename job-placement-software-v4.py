...
with st.form(key="placement_form"):
    ...
    services_options = {
        ...
        "Career Counseling": [
            ...
            "Achieving work or education goals"
        ],
        "Training Programs": [
        ...
        "Virtual and In-Person Services": [
            "Online services",
            "Services at physical locations across Minnesota"
        ]
    }

    selected_services = {}
    for service, options in services_options.items():
        if st.checkbox(service):
            selected_services[service] = st.multiselect(f"Select specific options for {service}", options)
...
if submit_button:
    if not full_name or not email or not phone or not skills:
        ...
        application_data = {
            ...
            "Selected Services": [str(selected_services)],
